...
def _banner():
    # Try to load external banner art if present to keep code lean
    here = os.path.abspath(os.path.dirname(__file__))
    project_root = os.path.dirname(here)
    banner_path = os.path.join(project_root, "image", "banner_cli.txt")
...
def run_analysis(input_csv: str, output_dir: str, location: Optional[str] = None, datum: Optional[str] = None) -> str:
    ...
    # Write cross-validation summary CSV
    cv_summary_path = os.path.join(output_dir, "cv_summary.csv")
    cv_rows: List[Dict[str, Any]] = []
    def _row(target: str, model: str, params: Dict[str, Any], metrics: Dict[str, float], buffer_radius: Optional[float]):
        row = {
            "target": target,
            "model": model,
            "k": params.get("k"),
            ...
            "buffer_radius": buffer_radius if buffer_radius is not None else "",
            "MAE": metrics.get("MAE"),
    ...
    _row("rock_top_elevation", "IDW (LOOCV)", {"k": k_rock, "p": p_rock}, rock_metrics, None)
    _row("limestone_thickness", "IDW (LOOCV)", {"k": k_thick, "p": p_thick}, thick_metrics, None)
    _row("rock_top_elevation", "IDW (buffered)", {"k": k_rock_bbuf, "p": p_rock_bbuf}, rock_bbuf_metrics, buffer_r_rock)
    _row("limestone_thickness", "IDW (buffered)", {"k": k_thick_bbuf, "p": p_thick_bbuf}, thick_bbuf_metrics, buffer_r_thick)
    _row("rock_top_elevation", "Trend + Aniso IDW (buffered)", {"k": k_rock_e, "p": p_rock_e, "theta": theta_rock, "ratio": ratio_rock}, rock_e_metrics, buffer_r_rock)
    _row("limestone_thickness", "Trend + Aniso IDW (buffered)", {"k": k_thick_e, "p": p_thick_e, "theta": theta_thick, "ratio": ratio_thick}, thick_e_metrics, buffer_r_thick)

    write_rows(cv_summary_path, [
        "target", "model", "k", "p", "theta", "ratio", "buffer_radius", "MAE", "RMSE", "R2"
    ], cv_rows)

    # Write markdown report with brief interpretation and actions
    report_path = os.path.join(output_dir, "report.md")
    def _counts_str(c: Dict[str, int]) -> str:
        ordered = ["measured", "predicted_enhanced", "predicted_idw", ""]
        parts = []
        for k in ordered:
            if k in c:
    ...
    report_lines = []
    report_lines.append("# Limestone Layer Analysis — Results Summary")
    report_lines.append("")
    if location:
        report_lines.append(f"- Site location: {location}")
    if datum:
        report_lines.append(f"- Depth datum: {datum}")
    report_lines.append(f"- Input file: {os.path.basename(input_csv)}")
    report_lines.append(f"- Output CSV: {os.path.basename(out_path)}")
    report_lines.append("")
    report_lines.append("## Cross-Validation Summary")
    report_lines.append("")
    def _fmt_metrics(lbl: str, params: str, m: Dict[str, float]):
        report_lines.append(f"- {lbl} ({params}): MAE={m['MAE']:.3f}, RMSE={m['RMSE']:.3f}, R2={m['R2']:.3f}")

    _fmt_metrics("Rock head — IDW LOOCV", f"k={k_rock}, p={p_rock}", rock_metrics)
    _fmt_metrics("Thickness — IDW LOOCV", f"k={k_thick}, p={p_thick}", thick_metrics)
    _fmt_metrics("Rock head — IDW buffered", f"k={k_rock_bbuf}, p={p_rock_bbuf}, buffer≈{round(buffer_r_rock,2)} m", rock_bbuf_metrics)
    _fmt_metrics("Thickness — IDW buffered", f"k={k_thick_bbuf}, p={p_thick_bbuf}, buffer≈{round(buffer_r_thick,2)} m", thick_bbuf_metrics)
    _fmt_metrics("Rock head — Trend+Aniso buffered", f"k={k_rock_e}, p={p_rock_e}, θ={theta_rock:.0f}°, ratio={ratio_rock}", rock_e_metrics)
    _fmt_metrics("Thickness — Trend+Aniso buffered", f"k={k_thick_e}, p={p_thick_e}, θ={theta_thick:.0f}°, ratio={ratio_thick}", thick_e_metrics)

    report_lines.append("")
    report_lines.append("## Data Provenance (counts)")
    report_lines.append("")
    report_lines.append("### rock_top_source")
    report_lines.append(_counts_str(rock_counts))
    report_lines.append("")
    report_lines.append("### sounding_source")
    report_lines.append(_counts_str(sounding_counts))
    report_lines.append("")
    report_lines.append("### thickness_source")
    report_lines.append(_counts_str(thickness_counts))

    report_lines.append("")
    report_lines.append("## Interpretation & Engineering Actions (brief)")
    report_lines.append("")
    report_lines.append("- Shallow rock head zones may lead to hard driving. Consider pre-drilling or heavier hammers.")
    report_lines.append("- Thin limestone pockets could overlie soft/karstic strata. Plan for deeper socketing or confirmatory tests.")
    report_lines.append("- Thick limestone areas may allow full end-bearing; evaluate coring costs vs capacity gains.")
    report_lines.append("- Where uncertainty is high (poorer CV metrics or sparse data), add boreholes/SPT/rock coring for verification.")

    with open(report_path, "w", encoding="utf-8") as rf:
        rf.write("\n".join(report_lines) + "\n")

    print(f"\nWrote CV summary to: {cv_summary_path}")
